...
/**
 * Centralized check for pages that require authentication.
 * Called from header.php to protect entire directory branches.
 */
function check_page_access(): void
{
    $script = $_SERVER['SCRIPT_NAME'] ?? '';
    $basename = basename($script);

    // Define which paths/files require a logged-in user
    $is_admin_area = (strpos($script, '/admin/') !== false);
    $is_protected_customer_page = (strpos($script, '/customer/') !== false && 
        in_array($basename, ['dashboard.php', 'cart.php', 'checkout.php', 'order_tracking.php'], true));

    if (($is_admin_area || $is_protected_customer_page) && !current_user_id()) {
        header('Location: ' . APP_URL . '/login.php');
        exit;
    }

    // Role-based directory protection
    if ($is_admin_area && current_user_role() !== 'admin') {
        http_response_code(403);
